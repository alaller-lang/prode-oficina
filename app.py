...
equipos_podio = sorted([
    ...
    "Sudáfrica", "Bosnia", "Catar", "Haití", "Escocia", "Turquía", "Curazao", "Costa de Marfil",
    "Túnez", "Egipto", "Irán", "Nueva Zelanda", "Cabo Verde", "Arabia Saudita", "Irak", "Noruega",
    "Austria", "Jordania", "RD Congo", "Uzbekistán", "Ghana", "Panamá", "Rep. Checa"
])
...
if nombre:
    tabs_main = st.tabs(["⚽ Fase de Grupos", "🏅 Podio Final", "💎 Bonus Especiales"])
    
    with st.form("prode_final_fix"):
        datos_enviar = []
        
        with tabs_main[0]:
            st.info("Completá los goles en cada grupo.")
        ...
        if st.form_submit_button("💾 GUARDAR PRONÓSTICO COMPLETO"):
            ...
            datos_enviar.append([nombre, "PODIO: 1° CAMPEÓN", p1, ""])
            datos_enviar.append([nombre, "PODIO: 2° SUBCAMPEÓN", p2, ""])
            datos_enviar.append([nombre, "PODIO: 3° TERCERO", p3, ""])
            datos_enviar.append([nombre, "PODIO: 4° CUARTO", p4, ""])
            datos_enviar.append([nombre, "BONUS: Último gol Arg", b1, ""])
            datos_enviar.append([nombre, "BONUS: Primer gol Octavos", b2, ""])
            
            ws_pronosticos.append_rows([f + [ahora] for f in datos_enviar])
            st.success("¡Guardado correctamente! Mucha suerte.")
            st.balloons()
else:
    st.info("Escribí tu nombre para ver el fixture.")
